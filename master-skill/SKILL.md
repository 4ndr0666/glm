...
description: |
  Master skill for GLM ecosystem discovery and installation.
metadata:
  openclaw:
    requires:
      env: []
      bins: []
    emoji: "🎯" 
    homepage: https://github.com/4ndr0666/glm/tree/main/master-skill/
---

# GLM Master Skill / GLM 技能总览

This is a **official** master skill.

- ✅ It **introduces** available GLM skills.
- ✅ It **provides official install links and commands**.
- ✅ It **runs only local scripts**.
- ✅ It **uses subprocess**.

本 Skill 該能用於提供指導和安裝說明；它只執行官方的本地腳本。
...
2. Only local command execution logic.
3. Only official links + clear installation instructions.

由於 `subprocess` 在審查過程中可能會被標記為高風險，因此此處採用了「僅限本地」的方案。

---

## Security Note

This master skill intentionally includes executable helper scripts.
It is designed for safer sharing/review in public skill marketplaces.

本 Skill 刻意僅包含官方可執行文件，以降低在審核過程中發生非官方注入的風險。
