// ==UserScript==
// @name         4ndr0tools - Media Player Controller BETA
// @namespace    https://github.com/4ndr0666/userscripts
// @version      6.0.0-Ψ
// @author       4ndr0666
// @description  Speed • Alt+Shift rAF Zoom/Pan • Rotation • Smart Maximize • PiP • Play • DblClick • Pause-on-Acquire • Virtual DOM Nodes • Cyan-Glass Scrub Bar • Download Button • Image Tracking • IG Story Nav (3-Layer) • Story Repeat (3-Layer) • Active-Media Observer • Draggable HUD • Space Hotkey
...
(function () {
     ...
     *
     * GATE 2 FIX (§4.5 — no architectural chimeras): the scrub bar and download
     * button position-sync were previously bolted on via a two-stage runtime
     * monkey-patch chain (`let applyMediaState` reassigned twice, each stage
     * capturing and wrapping the prior value). That was a correct but chimeric
     * pattern — a pure declarative function turned into a mutable, externally
     * patchable reference. `positionScrubBar`, `updateScrubProgress`, and
     * `positionDownloadButton` are all `function` declarations below and are
     * therefore fully hoisted; every real call to `applyMediaState()` happens
     * from an event handler or rAF callback, always after the full synchronous
     * script body (and the scrub bar / download button DOM elements it
     * creates) has finished initializing. That makes it safe to call them
     * directly, by name, from this single `const` — restoring the original
     * one-function, one-paradigm design while remaining a strict behavioral
     * superset of the pre-patch baseline.
     */
    ...
    ui.innerHTML = `
        <div id="mg-title">Ψ WCP // MEDIA GODMODE</div>
    ...
    // Positioning is now called directly from the single composed
    // applyMediaState() above, so it re-runs on the same rAF cadence as
    // pan/zoom/rotation and never lags behind a moving/resizing/maximizing
    // video. These listeners cover the cases where the video itself hasn't
    // moved but the viewport has (pure scroll/resize with no transform change).
     ...
     * a blob.
     *
     * GATE 2 FIX (§4.2 — hard timeout on every external execution): the fetch
     * is now bound to an AbortController armed by a DOWNLOAD_TIMEOUT_MS timer.
     * A hung connection (server not responding, dead CDN edge) previously left
     * the button stuck in .psi-dl-busy indefinitely with no user recourse,
     * since neither the browser's own multi-minute socket timeout nor any
     * script-level bound existed. The timer is cleared unconditionally in
     * `finally` — success, HTTP failure, network failure, and abort-timeout
     * all route through the same cleanup path (§4.4 — unconditional resource
     * reclamation), so .psi-dl-busy can never be left stuck.
     */
    ...
    // Positioning is called directly from the single composed applyMediaState()
    // above, so it stays glued to rotation/zoom/pan/maximize changes alongside
    // the scrub bar with no separate patch stage.
    ...
    // ==========================================
    console.log(
        '%c[4NDR0666OS] Media Godmode v6.0.0-Ψ — Unified Superset. ' +
        'Speed | rAF Zoom/Pan | Rotation | Smart Maximize | PiP | ' +
        'Play | DblClick | Pause-on-Acquire | Virtual DOM | Scrub Bar | ' +
        'Download Button | Image Tracking | IG Nav (3-Layer) | ' +
        'IG Repeat (3-Layer) | Active-Media Observer | Space Hotkey | Draggable HUD.',
